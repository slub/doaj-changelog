...
doaj_withdrawn_links <- readr::read_csv("data/doaj_changelog_withdrawn_list_links_via_cieps.csv", show_col_types = FALSE)
doaj_withdrawn_issn_l <- sort(unique(doaj_withdrawn_links$issn_l))
jsonlite::write_json(doaj_withdrawn_issn_l, "data/doaj_changelog_withdrawn_list_links.json", pretty = 2)
doaj_withdrawn_all_issns <- sort(unique(unlist(strsplit(doaj_withdrawn_links$all_issns, "\\|"))))
jsonlite::write_json(doaj_withdrawn_all_issns, "data/doaj_changelog_withdrawn_list_all_issns.json", pretty = 2)
